...
TEST(test_json_error_string_function) {
  ...
  ASSERT(strcmp(json_error_string((json_error)255), "Invalid error code") == 0); /* NOLINT */
  ASSERT_PTR_NOT_NULL(json_error_string(E_NO_ERROR));
  ASSERT_PTR_NOT_NULL(json_error_string((json_error)999)); /* NOLINT */
  END_TEST;
}
...
TEST(test_json_error_string_with_validate) {
  ...
  ASSERT(strcmp(json_error_string(error), "Malformed JSON structure") == 0);
